...
from config import original_data_to
from config import SortFields
...
def _modelling(items: List[Dict[str, Any]], spec: FieldSpec) -> List[DataModel]:
    ...
    Example:
        >>> data = TestData._modelling
        >>> spec = AppsFields().get_drg_wp_spec()
        >>> data_model = _modelling([data], spec)[0].model_dump()
        >>> del data_model['start'] # yamlに datetime.dateime は書き込めないので削除
        >>> del data_model['end']
        >>> print(data_model == Answer._modelling)
        True
    """
    _data_models = []
    for data in items:
...
--------------------------------------------------------------------------------
"""
def read_drg_way_point(fp: str, **kwargs) -> List[Dict[str, Any]]:
    """
    ## Description:
...
def read_gyoroman_gg2(fp: str, **kwargs) -> List[Dict[str, Any]]:
    """
...
def sort_models(
    ...
    DataModelをソートする
    Args:
        data_models(List[DataModel]): ソートするデータ
        sort_col(str): ソートするカラム名. `config.py`の`SortFields`を参照
        descending(bool): 降順ソートするかどうか。False: 昇順, True: 降順
    Returns:
        List[DataModel]: ソートされたデータ
    Example:
        >>> file_path = ".\\test\\point.gpx"
        >>> add_info = {'office': '青森', 'branch_office': '三厩', 'address': '100い1'}
        >>> data_models = read_drg_way_point(file_path, **add_info)
        >>> sort = SortFields.start.value
        >>> sorted_data_models = sort_models(data_models, sort, descending=True)
    """
    # Making dictionary. key: sort_col, value: index
    data = {}
    for i, model in enumerate(data_models):
        data[model.model_dump().get(sort_col)] = i
    # Sorting dictionary
    sorted_keys = sorted(data.keys(), reverse=descending)
    sorting_idx = [data[key] for key in sorted_keys]
    # Sorting DataModel
    sorted_models = []
    for i, idx in enumerate(sorting_idx):
        _data_model = data_models[idx]
...
if __name__ == "__main__":
    import doctest
    doctest.run_docstring_examples(_DrgWayPoint._read_coords, globals())
    doctest.run_docstring_examples(_DrgWayPoint.read_items, globals())
    doctest.run_docstring_examples(_modelling, globals())